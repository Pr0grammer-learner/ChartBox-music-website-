...
<body>
    ...
    <main>
        <section class="playlist">
            <h2 id='search-h2'>Поиск</h2>

            <!-- Форма для поиска -->
            <form action="search.php" method="get">
            <label id='search-label' for="search">Поиск по треку или артисту:</label>
            <input type="text" id="search" name="search" placeholder="Трек, Артист">     
            <button type="submit" id="search-button">Искать</button>
            </form>
            ...
            <?php
                ...
                if (isset($_GET['search'])) {
                    ...
                    // Запрос к базе данных для получения треков с совпадающими названиями или артистами
                    $sql = "SELECT * FROM tracks WHERE title LIKE '%$searchTerm%' OR artist LIKE '%$searchTerm%'";
                    $result = $conn->query($sql);
